...
const OUTPUT_SCHEMA = {
  ...
  properties: {
    ...
    data: {
      description: "Response data (present when ok=true). Structure varies by tool.",
    },
    error: {
      type: "object",
    ...
    meta: {
      ...
      properties: {
        ...
        httpStatus: { type: "number", description: "HTTP status code from pico-engine" },
        // Query-specific meta
        rid: { type: "string", description: "Ruleset ID (for queries)" },
        name: { type: "string", description: "Query function name (for queries)" },
        // Event-specific meta
...
const manifold_getThings = tool({
  ...
  required: ["eci"],
  outputDescription: "Returns map of thing picoIDs to thing objects (name, subID, picoID, color, etc.)",
});
...
const manifold_isAChild = tool({
  ...
  required: ["eci", "picoID"],
  outputDescription: "Returns boolean indicating if the given picoID is a child of the manifold pico",
});
...
const manifold_create_thing = tool({
  ...
  required: ["eci", "name"],
  outputDescription: "Event result (typically empty data object, check meta.httpStatus for success)",
});
...
const safeandmine_getTags = tool({
  ...
  properties: { ...TOOL_COMMON_PROPS },
  required: ["eci"],
  outputDescription: "Returns map of tag domains to tag IDs registered on this thing pico",
});
...
const safeandmine_delete = tool({
  ...
  properties: { ...TOOL_COMMON_PROPS, toDelete: { type: "string", description: "Field name to delete (name/email/phone/message). Omit to clear all." } },
  required: ["eci"],
  outputDescription: "Event result (typically empty data object, check meta.httpStatus for success)",
});
...
const addTags = tool({
  name: "addTags",
  description: "Installs safeandmine ruleset on a thing pico if not already installed",
  properties: { ...TOOL_COMMON_PROPS },
  required: ["eci"],
  outputDescription: "Returns undefined (void operation), check meta.httpStatus for success",
});
...
const childHasRuleset = tool({
  ...
  properties: {
    ...TOOL_COMMON_PROPS,
    rid: { type: "string", description: "Ruleset ID to check (e.g., io.picolabs.safeandmine)" },
  },
  required: ["eci", "rid"],
...
const installOwner = tool({
  ...
  properties: { ...TOOL_COMMON_PROPS },
  required: ["eci"],
  outputDescription: "Returns undefined (void operation), check console/logs for success",
});

const initializeManifold = tool({
  name: "initializeManifold",
  description: "Full bootstrap: install owner ruleset, create manifold pico, return manifold ECI (no args needed, uses root ECI)",
  properties: {},
  required: [],
  outputDescription: "Returns the manifold pico ECI as a string (not wrapped in uniform envelope)",
});
...
const installRuleset = tool({
  ...
  properties: {
    ...TOOL_COMMON_PROPS,
    filePath: { type: "string", description: "File URL (e.g., file:///path/to/ruleset.krl)" },
  },
